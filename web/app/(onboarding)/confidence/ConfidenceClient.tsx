...
const options = [
  ...
  { id: "unknown", label: "Unknown time", body: "Naksha uses noon and marks time-sensitive features as lower confidence." },
] as const;
...
export function ConfidenceClient({ replaceMode }: { replaceMode: boolean }) {
  ...
  function choose(confidence: string) {
    window.localStorage.setItem("naksha:birth_time_confidence", confidence);
      ...
      <section className="luxury-panel relative w-full max-w-4xl overflow-hidden rounded-lg p-6 sm:p-8">
        ...
        <div className="relative">
          ...
          <p className="mt-4 max-w-2xl text-sm leading-6 text-muted-foreground">
            Birth time controls house-sensitive interpretation. Naksha will label confidence instead of pretending uncertain data is exact.
          </p>
        </div>
